...
Owns everything the site says and shows that is not structure: projects, experience entries,
service entries, contact channels, and the per-locale copy dictionary.

All of it lives in typed files inside the repository, so editing content never means editing a
component. This capability also defines the two contracts that let the site ship before its
content is final: the `detailReady` flag that gates project detail links, and the placeholder
behaviour for imagery that has not been supplied yet.
