...
The entrance animation: content rises from below and fades in the first time it enters the
viewport, exactly once, staggered across a section's children.

Its defining constraint is that the animation is decorative only. Content must be present at
its resting position in server-rendered markup and in the accessibility tree whether or not the
animation ever runs — so a visitor without JavaScript, a crawler, or a failed bundle still sees
a finished page.
