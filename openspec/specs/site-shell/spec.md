...
The page's chrome and composition: the order of the seven regions, the sticky header with its
scroll-triggered backdrop, the navigation that marks the section currently being read, smooth
anchor scrolling offset for the header, the mobile hamburger overlay, the default footer, and
the responsive behaviour the single-breakpoint Figma never covered.
